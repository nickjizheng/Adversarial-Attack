...
# Data transformation for unimodal
...
# Dataset and DataLoader for unimodal
uni_dataset = HumanActivityDataset(data_dir, class_to_idx, transform=uni_transform)
uni_dataloader = DataLoader(uni_dataset, batch_size=1, shuffle=True)
...
# Unimodal functions
def uni_extract_keyframes(video_path, interval=30):
...
def uni_select_snippets(frames, snippet_length=16):
    snippets = []
...
def uni_create_adversarial_example(model, image_tensor, true_label_tensor, epsilon):
...
class UniAssistantModel:
    ...
    def process_images(self, frames, true_label, epsilon=0.1, attack=False):
        ...
        for frame in frames:
            image_tensor = self.uni_preprocess_image(frame)
            if image_tensor is None: 
                continue

            if attack:
                true_label_tensor = torch.tensor([true_label]).to(self.device)
                image_tensor = uni_create_adversarial_example(self.image_model, image_tensor, true_label_tensor, epsilon)
...
def uni_calculate_similarity(response1, response2):
...
def multi_select_class_samples(class_index, dataset):
...
# Unimodal Main Function
def uni_main(num_videos=1, specific_class=None):
    uni_model = ChatOpenAI(model=OPENAI_MODEL)
    uni_assistant = UniAssistant(uni_model)
    video_count = 0
    attempted_videos = 0
    shared_frames = []
    ...
    if specific_class is not None:
        ...
        uni_dataloader = multi_select_class_samples(specific_class, uni_dataset)

    for video_path, true_label in uni_dataloader:
        ...
        print(f"\nProcessing video {video_count + 1}/{num_videos}")
        ...
        interval = 5
        snippet_length = 4
        frames = uni_extract_keyframes(video_path, interval)
        shared_frames = frames  # Store shared frames
        print(f"Extracted {len(frames)} keyframes")
        snippets = uni_select_snippets(frames, snippet_length)
        ...
        uni_assistant_model = UniAssistantModel(image_model, device, uni_assistant)

        # Original Classification
        original_description = uni_assistant_model.process_images(selected_frames, true_label, attack=False)
        ...
        perturbed_description = uni_assistant_model.process_images(selected_frames, true_label, attack=True)
        print(f"Image Classification - Perturbed (Attacked): {perturbed_description}")

        # Generate Responses
        original_prompt = (f"In a paragraph:As your road safety monitoring assistant, the results of the traffic footage {original_description} to determine if the cyclist is safely navigating a roadway designated for vehicles, including bicycles, and identify any prohibited actions or potential dangers present?")

        perturbed_prompt = (f"In a paragraph:As your road safety monitoring assistant, the results of the traffic footage {perturbed_description} to determine if the cyclist is safely navigating a roadway designated for vehicles, including bicycles, and identify any prohibited actions or potential dangers present?")

        original_response = uni_assistant.answer(original_prompt)
        perturbed_response = uni_assistant.answer(perturbed_prompt)

        print(f"Response for Original Video: {original_response}")
        print(f"Response for Perturbed Video: {perturbed_response}")

        # Calculate similarity
        semantic_sim, syntax_sim = uni_calculate_similarity(original_response, perturbed_response)
        print(f"Semantic Similarity: {semantic_sim}")
        print(f"Syntax Similarity: {syntax_sim}")

        video_count += 1

    return shared_frames, true_label  # Return frames and label for multi-main

def multi_create_adversarial_example(model, image_tensor, true_label_tensor, epsilon):
    image_tensor.requires_grad = True
    output = model(image_tensor)
    true_label_tensor = true_label_tensor.squeeze()
    if true_label_tensor.dim() == 0:
...
class MultiAssistantModel:
    ...
    def multi_preprocess_image(self, image):
        if isinstance(image, np.ndarray):
            if image.ndim != 3 or image.shape[2] != 3:
                print(f"Invalid frame shape: {image.shape}. Skipping this frame.")
                return None
    ...
    def multi_process_images(self, frames, true_label, epsilon=0.1, attack=False):
        self.image_model.eval()
        descriptions = []
        encoded_frames = []

        for frame in frames:
            image_tensor = self.multi_preprocess_image(frame)
            if image_tensor is None:
                continue

            if attack:
                true_label_tensor = torch.tensor([true_label]).to(self.device)
                image_tensor = multi_create_adversarial_example(self.image_model, image_tensor, true_label_tensor, epsilon)
        ...
        description_votes = {desc: descriptions.count(desc) for desc in set(descriptions)}
        final_description = max(description_votes, key=description_votes.get)
        return final_description, encoded_frames
...
# Multimodal Main Function
def multi_main(shared_frames, true_label):
    model = ChatOpenAI(model=OPENAI_MODEL)
    video_count = 0
    print("\n--- MultiModal Processing ---")

    # Use the same shared_frames from uni_main
    multi_assistant_model = MultiAssistantModel(image_model, device)

    # Process all frames to get classifications for the attacked video
    # print("Attacking all frames...")
    attacked_description, encoded_frames = multi_assistant_model.multi_process_images(shared_frames, true_label, attack=True)

    if attacked_description is None:
        print("Error: No valid frames for classification after attack.")
        return

    # Prepare to evaluate the original video (without attack), using the same shared frames
    original_description, _ = multi_assistant_model.multi_process_images(shared_frames, true_label, attack=False)

    # Print results for both original and attacked descriptions
    # print(f"--- Classification Results for Multi ---")
    # print(f"Original Video Description: {original_description}")
    # print(f"Attacked Video Description: {attacked_description}")

    # Prepare questions for ChatGPT and process
    question = "In a paragraph: As your road safety monitoring assistant, can you evaluate the traffic footage to determine if the cyclist is safely navigating a roadway designated for vehicles, including bicycles, and identify any prohibited actions or potential dangers present?"

    # Limit number of images to send to ChatGPT (max 4 images)
    max_images_to_send = 4  
    selected_frames_for_chatgpt = encoded_frames[:max_images_to_send]  
    ...
    original_response = model([HumanMessage(content=prompt_original)]) 
    ...
    attacked_response = model([HumanMessage(content=prompt_attacked)]) 
    print(f"Response from ChatGPT (Attacked): {attacked_response.content}")

    # Calculate semantic and syntactic similarities
    semantic_sim, syntax_sim = uni_calculate_similarity(original_response.content, attacked_response.content)
...
if __name__ == "__main__":
    shared_frames, true_label = uni_main(num_videos=1, specific_class=10)  # Run unimodal main function and share frames
    multi_main(shared_frames, true_label)  # Run multimodal main function using the same frames
